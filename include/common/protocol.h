...
struct udp_header_t {
    ...
    std::uint32_t seq;        // 4 byte: số thứ tự gói
    std::uint32_t ack;        // 4 byte: số ACK (gói xác nhận)
    std::uint16_t payloadLen; // 2 byte: độ dài dữ liệu trong gói
    std::uint16_t checksum;   // 2 byte: checksum
};
#pragma pack(pop)
